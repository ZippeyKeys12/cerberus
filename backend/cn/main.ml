...
let maybe_generate_tests output_test_dir filename test_depth ail_prog prog5 tf =
...
let main
  ...
  try
    ...
    let result =
      ...
      maybe_generate_tests output_test_dir filename test_depth ail_prog prog5 testing_framework;
...
let output_test_dir =
  let doc = "TODO: Directory to place generated test suites in" in
  Arg.(value & opt (some string) None & info ["output-test-dir"] ~docv:"FILE" ~doc)

let test_depth =
  let doc = "TODO: Set the maximum depth of test suite generation" in
  Arg.(value & opt int 5 & info ["test-max-depth"] ~docv:"FILE" ~doc)

let testing_framework =
  let doc = "TODO: Specify the testing framework for test suite generation " in
  Arg.(value & opt (enum ["gtest", TestGeneration.GTest]) GTest & info ["testing-framework"] ~docv:"FILE" ~doc)
